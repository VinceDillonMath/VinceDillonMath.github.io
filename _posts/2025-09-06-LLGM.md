...
date: 2025-09-06
thumbnail: /assets/photos/Post-LLGM/length8Walks.png
excerpt: "The lamplighter group is well-loved in geometric group theory and related fields. This distribution came up in my study of random matrices associated to graphs, its moments are exactly the cogrowth sequence of the lamplighter group. I prove this and make some fun visualizations."
---
...
For the full grid of paths of length 8, click [here](www.vincedillon.com/assets/photos/Post-LLGM/gridLength8walks.png). 
...
Fix $k, N\in \mathbb{N}$. The value $\mu_k(A_n)$ is a random variable dependent on the diagonal Rademacher random variables. Hence, it can be viewed as a function, with abuse of notation, $\mu_k(A_n): \left\{-1,1\right\}^n\to \mathbb{R}$. We want to understand how much the moment changes if we flip one coordinate from $1$ to $-1$ or vice versa. Recall that we have
\begin{equation}
\mathrm{Tr}(A_n^k) = \sum_{i_1,i_2,\dots, i_k} A_n(i_1,i_2)A_n(i_2,i_3)\dots A_n(i_{k-1},i_k)A_n(i_ki_1).
\end{equation}

We now roughly bound the number of terms in this sum in which $A(i,i)$ appears for some fixed $1\leq i\leq n$. For even $k=2m$ the number of length-$k$ walks on the integers starting and ending at $0$ is ${2m\choose m}.$ We always assume $n\gg k$, so the number of these simple walks on the cycle is $n* {k \choose k/2}$, since we choose the base-point freely. Once we enforce that our path must visit $A(i,i)$, however, the dependence on $n$ disappears. This is because the path may start at most $k/2$ vertices away from $A(i,i)$ and so we have but $k$ choices of base point. Hence, $A(i,i)$ appears in at most $k{k \choose k/2}$ terms.

Since each term of the trace sum takes a value in $\{-1,0,1\}$, flipping the value of the $i^{th}$ coordinate changes the value of $\mathrm{Tr}(A_n^k)$ by at most $k {k\choose k/2}.$ After normalizing by $1/n$ (we're taking the average over $n$ eigenvalues), we see that flipping the $i^{th}$ coordinate changes the value of $\mu(A_n^k)$ by at most $\frac{k{k\choose k/2}}{n}$.

Applying McDiarmid's Inequality for $c_i= \frac{k{k\choose k/2}}{n}$ for all $i$, we see

\begin{equation}
    P(\lvert \mu_k(A_n)- \mathbb{E}[\mu_k(A_n)] \rvert \geq \varepsilon ) \leq 2\mathrm{exp}\left( -\frac{2\varepsilon^2n}{k{k\choose k/2}}\right)
\end{equation}
