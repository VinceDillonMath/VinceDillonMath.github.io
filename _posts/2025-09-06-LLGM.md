...
**<span style="color: #2869cb;">Lemma.</span>** (McDiarmid's Inequality)  *Let $f: \Chi_1\times \Chi_2 \times \cdots \times \Chi_n \to \mathbb{R}. Assume that changing the value of $x_i$ changes the value of $f$ by at most $c_i$. Let $X_1,X_2,\dots, X_n$ be independent random variables with $X_i\in \Chi_i$ for all $i$, then*
...
**<span style="color: #2869cb;">Theorem.</span>** *Let $\mu_k(A_n)$ denote the $k^{th}$ moment of the empirical spectral distribution of the matrix $A_n$. Then,*
\begin{equation}
    \lim_{n\to \infty} \mu_k(A_n)= \begin{cases}
        0& \mathrm{if k odd}\\
        b_{k/2} & \mathrm{if k even}
    \end{cases}
\end{equation}
