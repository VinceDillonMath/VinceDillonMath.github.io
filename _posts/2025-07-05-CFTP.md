...
\begin{equation}
    \frac{\mathrm{dim}_{\mathbb{F}_p} \mathrm{C}^{p}_K(K)-2}{2} \, \mathrm{>} \, \sqrt{\nu(K)+1}
\end{equation}
and thus
\begin{equation}
    \frac{(\mathrm{dim}_{\mathbb{F}_p} \mathrm{C}^p_K(K))^2}{4}\mathrm{>}\nu(K)+\mathrm{dim}_{\mathbb{F}_p} \mathrm{C}^p_K(K).
\end{equation}
Applying the above theorem gives the result.
