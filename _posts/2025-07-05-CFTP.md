...
The purpose of this post is to present a counterexample to the so-called class field tower problem. The flavor of the problem feels much more like pure algebra than the algebra that typically arises in number theory. So long as you're willing to black-box some result from class field theory, the problem also requires only a little algebraic number theory to undestand. Nonetheless, the problem remained open for decades (~1925-1965) despite being a natural question. We roughly follow [<span style="color: #ce9178;">Ers</span>] which can be found online [here](https://m-ershov.github.io/Research/gssurvey_revised.pdf). The counterexample we construct also provides a counterexample for the Burnside problem.
...
**<span style="color: #9cdcfe;">Corollary 1.</span>** If we can find $\tau\in (0,1)$ such that $1-\lvert U\rvert \tau +H_R(\tau)\leq 0,$ then $\text{Hilb}_A(t)$ diverges and hence $A$ is infinite-dimensional.
...
**<span style="color: #9cdcfe;">Corollary 2.</span>**  Let $\mathcal A$ be a finite-dimensional complete filtered algebra with minimal presentation $(U,R)$ and assume $\lvert U\rvert \geq 2.$ By minimal, we mean that no proper subset of $U$ generates $\mathcal A.$ We have
    \begin{equation}
...
**<span style="color: #9cdcfe;">Lemma.</span>** ([<span style="color: #ce9178;">Ers, p. 13</span>]) Let $G\cong \hat F_p(X)/N(R)$ be a pro-p group. Then $\lvert X\rvert =d(G)$ if and only if $R\subset \Phi(\hat F_p(X))$ if and only if $D(r)\geq 2 $ for all $r\in R.$

**<span style="color: #9cdcfe;">Corollary 3.</span>** ([<span style="color: #ce9178;">Koc</span>]) Let $G\cong \hat F_p(X)/N(R)$ be a pro-p group such that $X$ is minimal and finite. If $\lvert R\rvert \leq \lvert X\rvert ^2/4,$ then $G$ is infinite.
...
\begin{equation}
\mathrm{dim}_{\mathbb{F}_p} \mathrm{Cl}_{K}^{p}(K)>2+2\sqrt{\nu(K)+1},
\end{equation}
...
\begin{equation}
r(G_{K,p})\leq d(G_{K,p})^2/4= (\dim_{\mathbb{F}_p} \mathrm{C}^{p}_{K}(K))^2/4.
\end{equation}

By the previous theorem, we have 
\begin{equation}
r(G_{K,p})\leq \nu(K)+\mathrm{dim}_{\mathbb{F}_p}\mathrm{C}^p_K(K)-1.
\end{equation}

Assuming 
\begin{equation}
\mathrm{dim}_{\mathbb{F}_p}\mathrm{C}^p_K(K)>2+2\sqrt{\nu(k)+1},
\end{equation}

we obtain 
\begin{equation}
\frac{(\mathrm{dim}_{\mathbb{F}_p}\mathrm{C}^p_K(K))^2}{4}>\frac{(2+2\sqrt{\nu(K)+1})^2}{4}

=2+2\sqrt{\nu(K)+1}+\nu(K)+1

=\mathrm{dim}_{\mathbb{F}_p}\mathrm{C}^p_K(K)+\nu(K)+1
...
[Lan] S. Lang, “Algebra,” 3rd Edition, Springer Verlag, New York, 2002. doi:10.1007/978-1-4613-0041-0

[Ers] Ershov, Golod-Shafarevich Groups: A Survey. International Journal of Algebra and Computation, (2012)


[Wat] Waterhouse, Profinite Groups are Galois Groups, Procedings of the AMS, (1974)


[Koc] H. Koch, Galois theory of p-extensions, Springer Monograbs in Mathematics, 2001.

[Laz] M. Lazard, Groupes analytiques p-adiques. (French) Inst. Hautes Etudes Sci. Publ. Math. 26 (1965), ´
389-603.

[Sha] I. Shafarevich, Extensions with prescribed ramification points. (Russian) Inst. Hautes Etudes Sci. Publ. ´
