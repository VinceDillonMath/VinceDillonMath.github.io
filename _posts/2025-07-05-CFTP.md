...
**Theorem 3.** ([Koc]) Assume that $\dim_{\mathbb{F}_p} C_p(K)>2+2\sqrt{\nu(k)+1}$, then $G_{K,p}$ is infinite so that the class field tower of $K$ never stabilizes.

*Proof.* To apply Corollary 3, we want
    \begin{equation}
        r(G_{K,p})\leq d(G_{K,p})^2/4= (\dim_{\mathbb{F}_p} C_p(K))^2/4.
    \end{equation}
    By the previous theorem, we have $r(G_{K,p})\leq \nu(K)+\dim_{\mathbb{F}_p}C_p(K)-1.$  Assuming $\dim_{\mathbb{F}_p}C_p(K)>2+2\sqrt{\nu(k)+1}$, we obtain 
    \begin{align}
        \frac{(\dim_{\mathbb{F}_p}C_p(K))^2}{4}>\frac{(2+2\sqrt{\nu(K)+1})^2}{4}\\

        &=2+2\sqrt{\nu(K)+1}+\nu(K)+1\\

        &=\dim_{\mathbb{F}_p}C_p(K)+\nu(K)+1\\
